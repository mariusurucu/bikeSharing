    ...
    <section class="menu cid-qTkzRZLJNu" once="menu" id="menu1-0">
        ...
        <nav class="navbar navbar-expand beta-menu navbar-dropdown align-items-center navbar-fixed-top navbar-toggleable-sm">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <div class="navbar-buttons mbr-section-btn"><a class="btn btn-sm btn-secondary-outline display-4" href="userProfile.jsp"><span class="mbri-home mbr-iconfont mbr-iconfont-btn"></span>&nbsp;Home</a></div>
            </div>
        </nav>
    </section>
                    ...
                    <div class="table-responsive">
                        <table border="1" align="center">
                            ...
                            <%
                                ...
                                ResultSet rsStatus, rsStatus2,rs,rs2,rs3,rs4;
                                int docid,id_user=0;
                                String status = "In asteptare", mesaj = "In asteptare";
                                for (int i = 0; i < cookies.length; i++) {
                                    Cookie c = cookies[i];
                                    if (c.getName().equals("userid")) {
                                        rsStatus = jb.intoarceLinieDupaId("documents", "id_user", Integer.parseInt(c.getValue()));
                                        id_user = Integer.parseInt(c.getValue());
                                        if (rsStatus.next()) {
                                            docid = rsStatus.getInt("id");
                                            System.out.println("docid: " + docid);
                                            rsStatus2 = jb.intoarceLinieDupaId("flux", "docid", docid);
                                            if (rsStatus2.next()) {
                                                status = rsStatus2.getString("status");
                                                mesaj = rsStatus2.getString("mesaj");
                                            }
                                        }
                                        break;
                                    }
                                ...
                                rs = jb.intoarceLinieDupaId("application", "id_user", id_user);
                                int x;
                                if (rs.next()) {
                                    x = rs.getInt("id");
                                    ...
                                    if (rs2.next()) {
                                        ...
                                        if(rs3.next()){
                                            rs4 = jb.intoarceLinieDupaId("user", "userid", id_user);
                                            if(rs4.next()){
                            %>
